...
const SearchTrayScreen = () => {
    ...
    const handleSearch = () => {
        if (searchText.trim()) {
            ...
            // Replace the tray with the results screen so the tray doesn't persist in the stack
            router.replace(`/search?q=${encodeURIComponent(searchText.trim())}`);
        }
    };

    const handleTagPress = (query: string) => {
        addSearchQuery(query);
        // Replace to avoid leaving this modal route underneath subsequent screens
        router.replace(`/search?q=${encodeURIComponent(query)}`);
    };

    return (
        <View style={styles.container}>
            <BlurView intensity={Platform.OS === 'ios' ? 45 : 90} tint="dark" style={StyleSheet.absoluteFill} />
                            ...
                            <View style={styles.tagsContainer}>
                                {searchHistory.length > 0 ? (
                                    searchHistory.slice(0, 3).map((item, index) => (
                                        <BlurView key={index} intensity={Platform.OS === 'ios' ? 15 : 30} tint="extraLight" style={styles.tag}>
